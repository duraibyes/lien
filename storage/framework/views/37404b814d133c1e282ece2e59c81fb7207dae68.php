    ...
    <div class="create-project-form-bgcolor">
        ...
        <div class="contract--wrapper">
            ...
            <input class="form-control" type="number" name="job_no"
                value="<?php echo e(isset($contract) && $contract != '' && $contract->job_no != '' ? $contract->job_no : ''); ?>">
        </div>

        <div class="contract--wrapper">
            <label>General description of materials, services and/or labor furnished</label>
            <textarea class="form-control" type="text"
                name="general_description"><?php echo e(isset($contract) && $contract != '' && $contract->general_description != '' ? $contract->general_description : ''); ?></textarea>
        </div>

        <?php echo e(csrf_field()); ?>

        <input type="hidden" name="form_type" value="<?php echo e(isset($contract) && $contract != '' ? 'edit' : 'add'); ?>">
        <input type="hidden" name="project_id" value="<?php echo e($project_id); ?>" class="project_id">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#activate-step-3, #activate-step-3-out').on('click', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('project.contract.submit')); ?>",
                    data: data,
                    success: function(data) {
                        if (data.status) {
                            $.ajax({
                                type: "POST",
                                url: "<?php echo e(route('project.get.date')); ?>",
                                data: {
                                    tier: tier,
                                    state: state,
                                ...
                                success: function(data) {
                                    window.location.href = "<?php echo e($contactURL); ?>";
                                }
                            });
                        } else {
                            swal(
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    window.location.href =
                                        '<?php echo e(route('member.project')); ?>';
                                    buttonName.remove();
                                }
                            });
                        } else {
...
<?php echo $__env->make('basicUser.projects.create', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home2/lienmanager/public_html/resources/views/basicUser/projects/project_contract.blade.php ENDPATH**/ ?>