...
<?php $__env->startSection('body'); ?>
    ...
    <?php if(isset($_GET['edit'])): ?>
        ...
        <form id="document_form" action=<?php echo e(route('save.new.job.document')); ?> method="post"
            ...
            <div class="row">
                <?php if(isset($jobInfoSheet) && count($jobInfoSheet->jobFiles) > 0): ?>
                    <?php $__currentLoopData = $jobInfoSheet->jobFiles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="col-md-2" id="id<?php echo e($file->id); ?>">
                            <div class="fileRow projFileRow">
                                ...
                                <div class="col-xs-2 fileBtn"
                                    ...
                                    <button type="button" class="btn btn-xs btn-danger removeBtn"
                                        data-id="<?php echo e($file->id); ?>"><i class="fa fa-times"></i> </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="item">
                            <div class="form-group project_div" style="padding:20px;">
                                <input id="project_name" name="project_name" class="project_name"
                                    value="<?php echo e($project->project_name); ?>" hidden />
                                <input id="project_id" name="project_id" class="project_id" value="<?php echo e($project->id); ?>"
                                    hidden />
                            </div>
                        </div>
        ...
        <div class="col-md-12 col-sm-12 no-pad">
            <?php if(!isset($_GET['edit'])): ?>
                <h4 class="text-center">If you would like to complete a claim please select File A Claim, if not you may
                    save and quit.</h4>
                <?php if(isset($_GET['view']) && $_GET['view'] === 'detailed'): ?>
                    <a href="<?php echo e(route('member.create.edit.jobdescription', [$project_id]) . '?create=true&view=detailed'); ?>"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#saveDocuments', function(event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "<?php echo e(route('save.new.job.document')); ?>",
                    data: data,
                    success: function(data) {
                        window.location.href = "<?php echo e(route('create.deadlines', $project->id) . '?edit=true'); ?>";
                    }
                });
            });
        })
        ...
        Dropzone.options.documentDropzone = {
            url: "<?php echo e(route('add.job.file')); ?>",
            maxFilesize: 1024, // MB
            addRemoveLinks: true,
            headers: {
...
<?php echo $__env->make('basicUser.projects.create', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home2/lienmanager/public_html/resources/views/basicUser/projects/viewdocuments.blade.php ENDPATH**/ ?>